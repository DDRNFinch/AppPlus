...
const nav=()=>`<nav class="nav">${[['home','⌂','Home'],['academy','☆','Academy'],['apprenticeship','▣','Apprenticeship'],['ksb','◫','KSB Matrix'],['documents','▤','Documents'],['settings','⚙','Settings']].map(([id,i,l])=>`<button data-tab="${id}" class="${view.tab===id?'active':''}"><span class="ico">${i}</span>${l}</button>`).join('')}</nav>`;
...
function renderApprenticeship(){
  let c=course(),pct=Math.round(completedCount(c)/c.assignments.length*100),sub=['assignments','trade','witness'].includes(view.apprenticeshipTab)?view.apprenticeshipTab:'assignments';
...
function renderKsbMatrix(){
  ...
  const matrix=`<div class="card matrix-intro"><h2>KSB Matrix</h2><p class="muted">AS = assignment evidence, AT = Trade Academy quiz and WT = witness testimony. A KSB turns green only when each mapped assignment is complete and at least one matching supporting item—Trade Academy topic or witness testimony—is complete.</p><div class="matrix-legend"><span class="legend-assignment">AS completed</span><span class="legend-academy pending">AT not passed</span><span class="legend-academy passed">AT passed</span><span class="legend-witness pending">WT not added</span><span class="legend-witness passed">WT completed</span></div></div><div class="ksb-matrix">${rows.map(r=>{const done=r.assignments.length>0&&r.assignments.every(a=>assignmentSupportComplete(c,a));return `<div class="matrix-row ${done?'complete':''}"><div class="matrix-code">${done?'✓ ':''}${esc(r.code)}</div><div class="matrix-content"><h3>${esc(r.description||r.code)}</h3><div class="matrix-evidence"><div class="matrix-assignments">${r.assignments.map(a=>{const adone=!!state.completed[`${c.id}-${a.number}`],atId=`${c.id}-AT${a.number}`,atDone=!!state.academyPassed[atId],wt=state.witnessTestimonies[`${c.id}-WT${a.number}`];return `<button class="matrix-assignment ${adone?'complete':''}" data-matrix-a="${a.number}" title="${esc(a.title)}">${adone?'✓ ':''}AS${a.number}</button><button class="academy-support ${atDone?'passed':'pending'}" data-academy-topic="${esc(atId)}" title="Trade Academy: ${esc(a.title)}">${atDone?'✓ ':''}AT${a.number}</button><button class="witness-support ${wt?.completed?'passed':'pending'}" data-witness-topic="${a.number}" title="Witness testimony: ${esc(a.title)}">${wt?.completed?'✓ ':''}WT${a.number}</button>`}).join('')}</div></div></div></div>`}).join('')}</div>`;
  shell('KSB Matrix',`<div class="course-banner"><span>Current course</span><strong>${esc(c.name)}</strong></div>${matrix}`);
