...
function renderApprenticeship(){
  ...
  const practical=`<div class="course-banner practical-banner"><span>Practical Assessments</span><strong>${esc(c.name)}</strong></div><div class="card"><h2>Practical assessments</h2><p class="muted">Record the tutor's practical assessment, grade and feedback for each assignment.</p></div><div class="list">${c.assignments.map(a=>{const pa=state.practicalAssessments[`${c.id}-PA${a.number}`];return `<div class="assignment ${pa?.completed?'practical-done':''}" data-pa="${a.number}"><div class="num">${pa?.completed?'✓':`PA${a.number}`}</div><div class="grow"><h3>${esc(a.title)}</h3><div class="muted">${pa?.completed?`Completed · Grade ${pa.grade}/5`:'Not assessed'}</div></div><span>›</span></div>`}).join('')}</div>`;
...
function renderPracticalAssessment(){
  const c=course(),a=c.assignments.find(x=>x.number===Number(view.practicalAssessment))||c.assignments[0],pk=`${c.id}-PA${a.number}`;
  const d=state.practicalAssessments[pk]||{grade:'',feedback:'',assessor:'',date:new Date().toISOString().slice(0,10),completed:false};
  shell('Practical Assessment',`<button class="back" id="backPractical">‹ Back to Practical Assessments</button><div class="card"><span class="tag practical-tag">PA${a.number}</span><h2>${esc(a.title)}</h2><p class="muted">Mapped KSBs: ${(a.ksbs||[]).map(x=>esc(ksbCode(x))).join(', ')}</p></div><div class="card"><h2>Tutor assessment</h2><div class="form-row"><label>Practical grade</label><select id="practicalGrade"><option value="">Select grade</option>${[1,2,3,4,5].map(n=>`<option value="${n}" ${String(d.grade)===String(n)?'selected':''}>${n} — ${['','Below standard','Developing','Meets standard','Good standard','Excellent standard'][n]}</option>`).join('')}</select></div><div class="form-row"><label>Assessment feedback</label><textarea class="textarea practical-feedback" id="practicalFeedback" placeholder="Describe the practical work observed, the standard achieved and any improvements required.">${esc(d.feedback)}</textarea></div><div class="form-row"><label>Tutor or assessor</label><input id="practicalAssessor" value="${esc(d.assessor)}" placeholder="Full name"></div><div class="form-row"><label>Assessment date</label><input id="practicalDate" type="date" value="${esc(d.date)}"></div><button class="btn btn-primary" id="savePractical">Complete PA${a.number}</button></div>`);
  $('#backPractical').onclick=()=>{view.practicalAssessment=null;view.apprenticeshipTab='practical';render()};
  $('#savePractical').onclick=()=>{const grade=$('#practicalGrade').value,feedback=$('#practicalFeedback').value.trim(),assessor=$('#practicalAssessor').value.trim(),date=$('#practicalDate').value;if(!grade||!feedback||!assessor||!date){toast('Complete the grade, feedback, assessor and date');return}state.practicalAssessments[pk]={grade:Number(grade),feedback,assessor,date,completed:true,savedAt:new Date().toISOString()};save();toast(`PA${a.number} completed`);setTimeout(()=>{view.practicalAssessment=null;view.apprenticeshipTab='practical';render()},500)};
}
  ...
  for(let start=0;start<rows.length;start+=12){
    ...
    for(const r of rows.slice(start,start+12)){
      ...
      for(const a of r.assignments.slice(0,3)){
        const statuses=String(r.code).startsWith('S')?[['AS',!!state.completed[`${c.id}-${a.number}`],'#20bf75','#dce9e6'],['PA',!!state.practicalAssessments[`${c.id}-PA${a.number}`]?.completed,'#bc70cb','#f1dff5'],['WT',!!state.witnessTestimonies[`${c.id}-WT${a.number}`]?.completed,'#55aeea','#dcecf8']]:[['AS',!!state.completed[`${c.id}-${a.number}`],'#20bf75','#dce9e6'],['TA',!!state.academyPassed[`${c.id}-AT${a.number}`],'#f0c600','#fff3b8'],['WT',!!state.witnessTestimonies[`${c.id}-WT${a.number}`]?.completed,'#55aeea','#dcecf8']];
