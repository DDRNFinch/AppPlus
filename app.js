...
function renderDocuments(){
  ...
  const practicalEntries=course().assignments.filter(a=>state.practicalAssessments[`${view.courseId}-PA${a.number}`]?.completed);
  ...
  const practicalCards=practicalEntries.map(a=>{const d=state.practicalAssessments[`${view.courseId}-PA${a.number}`];return `<div class="document-card"><div><span class="tag practical-tag">PA${a.number}</span><h3>${esc(practicalTask(course(),a))}</h3><div class="muted">${d.result} · ${d.total}/50 · ${new Date(d.date+'T00:00:00').toLocaleDateString('en-GB')}</div></div><div class="document-actions"><button data-edit-pa="${a.number}">Edit</button><button data-download-pa="${a.number}">Download</button><button class="danger-mini" data-delete-pa="${a.number}">Delete</button></div></div>`}).join('');
