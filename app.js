...
function shell(title,body){$('#app').innerHTML=`<div class="app">${header(title)}<main class="main">${body}</main>${nav()}</div>`;$$('[data-tab]').forEach(b=>b.onclick=()=>{view.tab=b.dataset.tab;view.assignment=null;view.academyModule=null;view.academySection=null;view.witnessAssignment=null;view.practicalAssessment=null;save();render()})}
function render(){if(view.assignment)return renderAssignment();if(view.academyModule)return renderAcademyModule();if(view.practicalAssessment)return renderPracticalAssessment();if(view.witnessAssignment)return renderWitnessTestimony();({home:renderHome,academy:renderAcademy,apprenticeship:renderApprenticeship,ksb:renderKsbMatrix,documents:renderDocuments,settings:renderSettings}[view.tab]||renderHome)()}
