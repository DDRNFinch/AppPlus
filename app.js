...
function assignmentSupportComplete(c,a){const asDone=!!state.completed[`${c.id}-${a.number}`];const taDone=!!state.academyPassed[`${c.id}-AT${a.number}`];const paDone=!!state.practicalAssessments[`${c.id}-PA${a.number}`]?.completed;const wtDone=!!state.witnessTestimonies[`${c.id}-WT${a.number}`]?.completed;return asDone&&(taDone||paDone||wtDone)}
...
function renderKsbMatrix(){
  ...
  const matrix=`<div class="card matrix-intro"><h2>KSB's</h2><p class="muted">AS = assignment statement, TA = Trade Academy, PA = practical assessment and WT = witness testimony. A KSB turns green when its assignment is complete with supporting TA, PA or WT evidence.</p><div class="matrix-legend"><span class="legend-assignment">AS completed</span><span class="legend-academy pending">TA not passed</span><span class="legend-academy passed">TA passed</span><span class="legend-practical pending">PA not completed</span><span class="legend-practical passed">PA completed</span><span class="legend-witness pending">WT not added</span><span class="legend-witness passed">WT completed</span></div></div><div class="ksb-matrix">${rows.map(r=>{const done=r.assignments.length>0&&r.assignments.every(a=>assignmentSupportComplete(c,a));return `<div class="matrix-row ${done?'complete':''}"><div class="matrix-code">${done?'✓ ':''}${esc(r.code)}</div><div class="matrix-content"><h3>${esc(r.description||r.code)}</h3><div class="matrix-evidence"><div class="matrix-assignments">${r.assignments.map(a=>{const adone=!!state.completed[`${c.id}-${a.number}`],taId=`${c.id}-AT${a.number}`,taDone=!!state.academyPassed[taId],pa=state.practicalAssessments[`${c.id}-PA${a.number}`],wt=state.witnessTestimonies[`${c.id}-WT${a.number}`];return `<button class="matrix-assignment ${adone?'complete':''}" data-matrix-a="${a.number}" title="${esc(a.title)}">${adone?'✓ ':''}AS${a.number}</button><button class="academy-support ${taDone?'passed':'pending'}" data-academy-topic="${esc(taId)}" title="Trade Academy: ${esc(a.title)}">${taDone?'✓ ':''}TA${a.number}</button><button class="practical-support ${pa?.completed?'passed':'pending'}" data-practical-topic="${a.number}" title="Practical assessment: ${esc(a.title)}">${pa?.completed?'✓ ':''}PA${a.number}</button><button class="witness-support ${wt?.completed?'passed':'pending'}" data-witness-topic="${a.number}" title="Witness testimony: ${esc(a.title)}">${wt?.completed?'✓ ':''}WT${a.number}</button>`}).join('')}</div></div></div></div>`}).join('')}</div>`;
...
async function matrixPdfBytes(c){
  ...
  for(let start=0;start<rows.length;start+=12){
    ...
    for(const r of rows.slice(start,start+12)){
      const done=r.assignments.length&&r.assignments.every(a=>assignmentSupportComplete(c,a));ctx.fillStyle=done?'#dff8e6':'#f4f8f7';ctx.fillRect(55,y-28,1130,100);ctx.fillStyle='#073f43';ctx.font='bold 23px Arial';ctx.fillText(`${done?'✓ ':''}${r.code}`,75,y);ctx.font='18px Arial';wrapCanvasText(ctx,r.description||r.code,165,y,570,24,2);let bx=755;
      for(const a of r.assignments.slice(0,3)){
        const statuses=[['AS',!!state.completed[`${c.id}-${a.number}`],'#20bf75','#dce9e6'],['TA',!!state.academyPassed[`${c.id}-AT${a.number}`],'#f0c600','#fff3b8'],['PA',!!state.practicalAssessments[`${c.id}-PA${a.number}`]?.completed,'#bc70cb','#f1dff5'],['WT',!!state.witnessTestimonies[`${c.id}-WT${a.number}`]?.completed,'#55aeea','#dcecf8']];
