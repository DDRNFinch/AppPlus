...
function practicalCriteria(a){
  const generic=['Safe working and correct PPE','Setting out and dimensions','Selection and preparation of tools and materials','Accuracy and alignment','Level','Plumb','Gauge and consistent spacing','Quality of joints and connections','Range, tolerances and specification','Finished appearance and housekeeping'];
...
function renderPracticalAssessment(){
  const c=course(),a=c.assignments.find(x=>x.number===Number(view.practicalAssessment))||c.assignments[0],pk=`${c.id}-PA${a.number}`,criteria=practicalCriteria(a);
...
function practicalPdf(cid,num){
  ...
  const criteria=practicalCriteria(a),rows=criteria.map((x,i)=>`<tr><td>${i+1}</td><td>${esc(x)}</td><td><b>${d.scores?.[i]||0}/5</b></td></tr>`).join('');
