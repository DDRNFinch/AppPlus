(function(){
  ...
  function rplQuestion(row,rows,c){
    const peers=rows.filter(x=>x.code!==row.code&&x.code[0]===row.code[0]);
    const fallback=rows.filter(x=>x.code!==row.code),wrong=[...peers,...fallback].filter((x,i,a)=>a.findIndex(y=>y.code===x.code)===i).slice(0,3).map(x=>x.description);
    while(wrong.length<3)wrong.push(['This requirement can be ignored when the task is familiar.','No evidence or quality check is needed once work has started.','The learner should continue beyond their authority instead of escalating.'][wrong.length]);
    const correct=row.description,answer=(Number(row.code.slice(1))||0)%4,options=[...wrong];options.splice(answer,0,correct);
    return {q:`Which statement best describes ${row.code} within ${c.name}?`,options,answer,correct};
  }
  ...
  function renderRplItem(){
    ...
    shell('RPL Refresher',`<button class="back" id="backRplSession">‹ End refresher</button><div class="course-banner refresh-banner"><span>${esc(c.name)}</span><strong>Topic ${number} of ${codes.length}</strong></div><article class="card refresh-learning-card"><span>RPL TOPIC · ${esc(row.code)}</span><h2>${esc(row.code)}</h2><p>${esc(row.description)}</p><h3>${esc(q.q)}</h3><div class="refresh-answer-grid">${q.options.map((o,i)=>`<button data-rpl-answer="${i}">${esc(o)}</button>`).join('')}</div><div class="refresh-result" id="rplRefreshResult">Choose the best answer. There is no pass or fail.</div><button class="btn btn-primary" id="nextRplItem" hidden>${number===codes.length?'Finish refresher':'Next topic'}</button></article>`);
